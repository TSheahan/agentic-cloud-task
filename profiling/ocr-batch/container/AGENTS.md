...
| [run-local-smoke.sh](run-local-smoke.sh) | `docker run` with `OCR_LOCAL_FILE` / `OCR_LOCAL_OUTPUT_DIR`; mounts repo at `/work`; default input `test-media/service-invoice.jpg` → `smoke-out/` (see [Audit §13](../ocr-batch.profile.md)) |
