#!/usr/bin/env python3
"""Round 5: RapidOCR `backend="torch"` + Docling CUDA only; spacing fix on export.

Same inputs as round 2 (`WORKDIR/sample_scan/`). Differs from round 2 by:
  - No CPU accelerator pass.
  - `rapidocr_params={"Rec.use_space_char": True}` (with torch + full-page OCR).
...
Usage (on instance, venv active):
  ...
  python r5-torch-spacing.py /path/to/run/dir
"""
...
def main() -> None:
    if len(sys.argv) < 2:
        print(
            "Usage: python r5-torch-spacing.py <run_directory>",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    payload = {
        ...
        "methodology": (
            "Round 5: RapidOCR torch + full-page OCR + table structure; "
