#!/usr/bin/env python3
"""Round 2: RapidOCR `backend="torch"` + Docling CUDA vs CPU accelerator sweep.

Same inputs as round 1 (`WORKDIR/sample_scan/`). Differs by:
  - `RapidOcrOptions(backend="torch", force_full_page_ocr=True)` (torch uses GPU when available)
...
Usage (on instance, venv active):
  ...
  python r2-torch.py /path/to/run/dir

Requires PyTorch with CUDA in the venv for GPU RapidOCR inference.
"""
...
def main() -> None:
    if len(sys.argv) < 2:
        print(
            "Usage: python r2-torch.py <run_directory>",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    payload = {
        ...
        "methodology": (
            "Round 2: RapidOCR torch backend (GPU when available) + full-page OCR + "
            "table structure; Docling accelerator CUDA vs CPU for layout/table ONNX. "
            "Same cold/warm pattern as round 1."
        ),
